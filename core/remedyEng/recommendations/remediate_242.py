    ...
    def remediate(self) -> None:
        ...
        for file_path, file_data in self.child_ast_config.items():
            ...
            for remediation in self.child_scan_result[file_path]:
                ...
                # add_block action: create catch-all server block
                if action == "add_block" and directive == "server":
                    rel_ctx = self._relative_context(context)
                    if not rel_ctx:
                        http_blocks = self._find_block_contexts(parsed_copy, "http")
                        if http_blocks:
                    ...
                    patches.append({
                        ...
                        "exact_path": rel_ctx,
                        "directive": "server",
                        "block": default_server_block,
                        "priority": position_hint if position_hint == 0 else 1,
                    })
                
                # add_directive action: add return/ssl_reject to existing block
                elif action in {"add", "add_directive"} and directive in {"return", "ssl_reject_handshake"}:
                    rel_ctx = self._relative_context(context)
                    args = remediation.get("args", [])
                    if rel_ctx and isinstance(args, list):
                        patches.append({
                            "action": "upsert",
                            "exact_path": rel_ctx,
                            "directive": directive,
