...
class Detector531(BaseRecom):
    ...
    def _evaluate_block(self, block_type: str, dirs_at_level: List[Any], exact_path: List[Any], inherited_state: str, logical_context: List[str], http_state: str, config_list: List[Dict[str, Any]], all_remediations: List[Dict[str, Any]], unique_remediations: set):
        current_state, invalid_headers = self._get_headers_state(dirs_at_level, inherited_state)
        
        # 1. Generate REPLACE remediations for invalid headers in this block
        for d, f_idx, d_path in invalid_headers:
            self._add_remediation(f_idx, "replace", "add_header", ["X-Content-Type-Options", '"nosniff"', "always"], logical_context, d_path, config_list, all_remediations, unique_remediations)
        
        # 2. Check if we need to ADD
        if current_state == "MISSING" and not invalid_headers:
            has_location = any(d.get("directive") == "location" for d, _, _ in dirs_at_level)
            should_add = False
            ...
            if should_add:
                f_idx = exact_path[1]
                self._add_remediation(f_idx, "add", "add_header", ["X-Content-Type-Options", '"nosniff"', "always"], logical_context, exact_path, config_list, all_remediations, unique_remediations)
        ...
        for d, f_idx, d_path in dirs_at_level:
            if "block" in d:
                ...
                if new_block_type in ("http", "server", "location"):
                    ...
                    self._evaluate_block(new_block_type, new_dirs_at_level, new_exact_path, pass_state, new_logical_context, http_state, config_list, all_remediations, unique_remediations)
    ...
    def scan(self, parser_output: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for f_idx, config in enumerate(config_list):
            for i, d in enumerate(config.get("parsed", [])):
                if d.get("directive") == "http":
                    ...
                    http_dirs = list(self._get_directives_at_level(d.get("block", []), d_path, config_list))
                    http_state, _ = self._get_headers_state(http_dirs, "MISSING")
                    break
        ...
        for f_idx, config in enumerate(config_list):
            ...
            self._evaluate_block("main", root_dirs, root_exact_path, "MISSING", [], http_state, config_list, all_remediations, unique_remediations)
