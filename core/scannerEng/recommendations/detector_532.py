...
class BlockNode:
    def __init__(self, block_type: str, exact_path: List[Any], f_idx: int):
        ...
        self.has_valid_csp = False
        self.invalid_csp = []
        self.clean_descendants = True
        self.needs_csp_directly = False
        ...
        for d, c_f_idx, c_path in dirs_at_level:
            ...
            if dir_name == "add_header":
                ...
                if args and args[0].lower() in ("content-security-policy", "content-security-policy-report-only"):
                    if self._check_csp(d):
                        node.has_valid_csp = True
                    else:
                        node.invalid_csp.append((d, c_f_idx, c_path))
            elif "block" in d:
