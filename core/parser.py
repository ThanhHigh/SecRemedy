...
class NginxParser:
    ...
    def parse(self):
        ...
        if payload.get("status") == "failed" or payload.get("errors"):
            print("[CẢNH BÁO] Crossplane gặp lỗi (Thường do sai đường dẫn include tuyệt đối):")
...
if __name__ == "__main__":
    ...
    parser_cli = argparse.ArgumentParser(description="Nginx Configuration AST Parser (Crossplane Wrapper)")
    
    # Thêm tham số -P hoặc --port (Bắt buộc)
    parser_cli.add_argument("-P", "--port", required=True, help="Port của Nginx Server đã được fetch (VD: 2221, 2222)")
    
    # Thêm tham số -o hoặc --output (Tùy chọn, nếu không truyền sẽ tự sinh tên theo port)
    parser_cli.add_argument("-o", "--output", help="Đường dẫn file JSON output (Tùy chọn)")
    ...
    if not os.path.exists(TARGET_DIR):
        ...
        print(f"[*] Gợi ý: Hãy chạy lệnh 'python core/fetcher.py -P {args.port}' trước để tải cấu hình về máy.")
        exit(1)
    ...
    try:
        ...
        print(f"[*] Tổng số file cấu hình đã phân tích thành công: {parsed_files}")
