...
def test_valid_port_80(detector):
    ...
    out = _make_parser_output([_http_block([_server_block([_dir("listen", ["80"])])])])
    assert detector.scan(out) == []

def test_valid_port_443(detector):
    """Test 5: Chỉ chứa listen 443"""
    out = _make_parser_output([_http_block([_server_block([_dir("listen", ["443"])])])])
    assert detector.scan(out) == []

def test_valid_port_8080(detector):
    """Test 6: Chỉ chứa listen 8080"""
    out = _make_parser_output([_http_block([_server_block([_dir("listen", ["8080"])])])])
    assert detector.scan(out) == []
...
def test_invalid_port_8000(detector):
    ...
    out = _make_parser_output([_http_block([_server_block([_dir("listen", ["8000"])])])])
...
def test_invalid_port_8001(detector):
    """Test 10: listen 8001"""
    out = _make_parser_output([_http_block([_server_block([_dir("listen", ["8001"])])])])
    assert len(detector.scan(out)[0]["remediations"]) == 1

def test_invalid_port_8444(detector):
    """Test 11: listen 8444"""
    out = _make_parser_output([_http_block([_server_block([_dir("listen", ["8444"])])])])
    assert len(detector.scan(out)[0]["remediations"]) == 1

def test_invalid_port_21(detector):
    """Test 12: listen 21"""
    out = _make_parser_output([_http_block([_server_block([_dir("listen", ["21"])])])])
    assert len(detector.scan(out)[0]["remediations"]) == 1

def test_invalid_port_22(detector):
    """Test 13: listen 22"""
    out = _make_parser_output([_http_block([_server_block([_dir("listen", ["22"])])])])
    assert len(detector.scan(out)[0]["remediations"]) == 1
...
def test_mixed_valid_invalid_80_8081(detector):
    ...
    assert len(res[0]["remediations"]) == 1
    # index 1 là 8081
    assert res[0]["remediations"][0]["exact_path"] == ["config", 0, "parsed", 0, "block", 0, "block", 1]
...
def test_invalid_port_4444(detector):
    """Test 16: listen 4444"""
    out = _make_parser_output([_http_block([_server_block([_dir("listen", ["4444"])])])])
    assert len(detector.scan(out)[0]["remediations"]) == 1

def test_invalid_port_6379(detector):
    """Test 17: listen 6379"""
    out = _make_parser_output([_http_block([_server_block([_dir("listen", ["6379"])])])])
    assert len(detector.scan(out)[0]["remediations"]) == 1

def test_invalid_port_27017(detector):
    """Test 18: listen 27017"""
    out = _make_parser_output([_http_block([_server_block([_dir("listen", ["27017"])])])])
    assert len(detector.scan(out)[0]["remediations"]) == 1
...
def test_valid_ip_127_0_0_1_80(detector):
    ...
    out = _make_parser_output([_http_block([_server_block([_dir("listen", ["127.0.0.1:80"])])])])
    assert detector.scan(out) == []

def test_valid_ip_192_168_1_1_443(detector):
    """Test 20: 192.168.1.1:443"""
    out = _make_parser_output([_http_block([_server_block([_dir("listen", ["192.168.1.1:443"])])])])
    assert detector.scan(out) == []

def test_valid_ip_0_0_0_0_8080(detector):
    """Test 21: 0.0.0.0:8080"""
    out = _make_parser_output([_http_block([_server_block([_dir("listen", ["0.0.0.0:8080"])])])])
    assert detector.scan(out) == []

def test_invalid_ip_127_0_0_1_8000(detector):
    """Test 22: 127.0.0.1:8000"""
    out = _make_parser_output([_http_block([_server_block([_dir("listen", ["127.0.0.1:8000"])])])])
    assert len(detector.scan(out)[0]["remediations"]) == 1

def test_invalid_ip_10_0_0_1_8081(detector):
    """Test 23: 10.0.0.1:8081"""
    out = _make_parser_output([_http_block([_server_block([_dir("listen", ["10.0.0.1:8081"])])])])
    assert len(detector.scan(out)[0]["remediations"]) == 1

def test_invalid_ip_0_0_0_0_22(detector):
    """Test 24: 0.0.0.0:22"""
    out = _make_parser_output([_http_block([_server_block([_dir("listen", ["0.0.0.0:22"])])])])
    assert len(detector.scan(out)[0]["remediations"]) == 1

def test_valid_ip_no_port(detector):
    """Test 25: 127.0.0.1 (mặc định 80)"""
    out = _make_parser_output([_http_block([_server_block([_dir("listen", ["127.0.0.1"])])])])
    assert detector.scan(out) == []
...
def test_invalid_ipv6_specific_8002(detector):
    ...
    out = _make_parser_output([_http_block([_server_block([_dir("listen", ["[2001:db8::1]:8002"])])])])
    assert len(detector.scan(out)[0]["remediations"]) == 1
...
def test_valid_args_443_ssl_http2(detector):
    ...
    out = _make_parser_output([_http_block([_server_block([_dir("listen", ["443", "ssl", "http2"])])])])
    assert detector.scan(out) == []

def test_valid_args_80_default_server(detector):
    """Test 32: 80 default_server"""
    out = _make_parser_output([_http_block([_server_block([_dir("listen", ["80", "default_server"])])])])
    assert detector.scan(out) == []

def test_valid_args_443_quic_reuseport(detector):
    """Test 33: 443 quic reuseport"""
    out = _make_parser_output([_http_block([_server_block([_dir("listen", ["443", "quic", "reuseport"])])])])
    assert detector.scan(out) == []

def test_invalid_args_8444_ssl_http2(detector):
    """Test 34: 8444 ssl http2"""
    out = _make_parser_output([_http_block([_server_block([_dir("listen", ["8444", "ssl", "http2"])])])])
    assert len(detector.scan(out)[0]["remediations"]) == 1

def test_invalid_args_ipv6_8000_default(detector):
    """Test 35: [::]:8000 default_server"""
    out = _make_parser_output([_http_block([_server_block([_dir("listen", ["[::]:8000", "default_server"])])])])
    assert len(detector.scan(out)[0]["remediations"]) == 1
    ...
    out = {
        ...
        "config": [
            ...
            },
            {
                ...
                "status": "ok", "errors": [],
                "parsed": [_server_block([_dir("listen", ["8001"])])] # Lưu ý file con có thể không cần bọc http
            }
        ]
    }
    res = detector.scan(out)
...
def test_nested_no_listen(detector):
    ...
    out = _make_parser_output([_http_block([_server_block([_dir("server_name", ["localhost"])])])])
    assert detector.scan(out) == []

def test_nested_unix_socket(detector):
    """Test 43: unix socket"""
    out = _make_parser_output([_http_block([_server_block([_dir("listen", ["unix:/var/run/nginx.sock"])])])])
    assert detector.scan(out) == []
