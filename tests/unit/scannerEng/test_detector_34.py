    ...
    out = _make_parser_output([_http_block([
        _server_block([
            _location_block(["~", "\\.php$"], [
                ...
                _dir("fastcgi_param", ["X-Forwarded-For", "$proxy_add_x_forwarded_for"]),
    ...
    out = _make_parser_output([_http_block([
        _server_block([
            _location_block(["/"], [
                ...
                _dir("grpc_set_header", ["X-Forwarded-For", "$proxy_add_x_forwarded_for"]),
...
def test_multifile_app_missing(detector):
    ...
    out = {
        "status": "ok", "errors": [], "config": [
            {"file": "nginx.conf", "status": "ok", "errors": [], "parsed": [_http_block([_dir("include", ["conf.d/*.conf"])])]},
            {"file": "app.conf", "status": "ok", "errors": [], "parsed": [_server_block([_location_block(["/"], [_dir("proxy_pass", ["http://b"])])])]}
        ]
    }
    res = detector.scan(out)
    assert len(res) == 1
...
def test_multifile_one_missing(detector):
    ...
    out = {
        "status": "ok", "errors": [], "config": [
            ...
            {"file": "2.conf", "status": "ok", "errors": [], "parsed": [_server_block([_location_block(["/"], [_dir("proxy_pass", ["http://b"])])])]}
        ]
    }
    res = detector.scan(out)
    assert len(res) == 1
...
def test_multifile_level2_include(detector):
    ...
    out = {
        "status": "ok", "errors": [], "config": [
            {"file": "nginx.conf", "status": "ok", "errors": [], "parsed": [_http_block([_dir("include", ["app.conf"])])]},
            {"file": "app.conf", "status": "ok", "errors": [], "parsed": [_server_block([_dir("include", ["api.conf"])])]},
            {"file": "api.conf", "status": "ok", "errors": [], "parsed": [_location_block(["/"], [_dir("proxy_pass", ["http://b"])])]}
        ]
    }
    res = detector.scan(out)
    assert len(res) == 1
...
def test_multifile_two_includes_missing(detector):
    ...
    out = {
        "status": "ok", "errors": [], "config": [
            {"file": "app1.conf", "status": "ok", "errors": [], "parsed": [_server_block([_location_block(["/1"], [_dir("proxy_pass", ["http://b1"])])])]},
            {"file": "app2.conf", "status": "ok", "errors": [], "parsed": [_server_block([_location_block(["/2"], [_dir("proxy_pass", ["http://b2"])])])]}
        ]
    }
    res = detector.scan(out)
    assert len(res) == 2
