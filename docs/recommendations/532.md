# _5.3.2 Ensure that Content Security Policy (CSP) is enabled and configured properly (Manual)_
...
**C**ontent **S**ecurity **P**olicy (CSP) is an HTTP response header that allows site administrators to declare approved sources of content that browsers are allowed to load on that page. It is a mechanism to detect and mitigate certain types of attacks, including **C**ross-**S**ite **S**cripting (XSS) and data injection attacks. Furthermore, CSP's frameancestors directive is the modern replacement for the X-Frame-Options header to prevent Clickjacking.
...
#### [OWASP HTTP Headers Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/HTTP_Headers_Cheat_Sheet.html) states:
